import { Skeleton } from '~/components/ui/skeleton';
import { IconType } from 'react-icons';
import { VariantProps, cva } from 'class-variance-authority';
import { cn, formatCurrency, formatPercentage } from '~/lib/utils';
import { Card, CardTitle, CardDescription, CardContent, CardHeader } from '~/components/ui/card';
import { CountUp } from './count-up';

const boxVariants = cva('rounded-lg p-4', {
  variants: {
    variant: {
      default: 'bg-blue-500/20',
      success: 'bg-green-500/20',
      warning: 'bg-yellow-500/20',
      danger: 'bg-red-500/20',
    },
  },
  defaultVariants: {
    variant: 'default',
  },
});

const iconVariants = cva('size-6', {
  variants: {
    variant: {
      default: 'fill-blue-500',
      success: 'fill-green-500',
      warning: 'fill-yellow-500',
      danger: 'fill-red-500',
    },
  },
  defaultVariants: {
    variant: 'default',
  },
});

type BoxVariants = VariantProps<typeof boxVariants>;
type IconVariants = VariantProps<typeof iconVariants>;

interface DataCardProps extends BoxVariants, IconVariants {
  title: string;
  value?: number;
  percentageChange?: number;
  range: string;
  icon: IconType;
}

export const DataCard = ({ icon: Icon, range, title, percentageChange = 0, value = 0, variant }: DataCardProps) => {
  return (
    <Card className='border-none drop-shadow-sm'>
      <CardHeader className='flex flex-row gap-x-4 items-center justify-between'>
        <div className='space-y-2'>
          <CardTitle className='text-2xl line-clamp-2'>{title}</CardTitle>
          <CardDescription className='line-clamp-1'>{range}</CardDescription>
        </div>
        <div className={cn(boxVariants({ variant }))}>
          <Icon className={cn(iconVariants({ variant }))} />
        </div>
      </CardHeader>
      <CardContent>
        <h1 className='font-bold text-2xl mb-2 line-clamp-1 break-all'>
          <CountUp preserveValue end={value} start={0} decimals={2} decimalPlaces={2} formattingFn={formatCurrency} />
        </h1>
        <p
          className={cn(
            'text-sm text-blue-500 line-clamp-1 break-all',
            percentageChange > 0 && 'text-green-500',
            percentageChange < 0 && 'text-red-500'
          )}
        >
          {formatPercentage(percentageChange, { addPrefix: true })} from last period
        </p>
      </CardContent>
    </Card>
  );
};

export const DataCardSkeleton = () => {
  return (
    <Card className='border-none drop-shadow-sm h-[192px]'>
      <CardHeader className='flex flex-row gap-x-4 items-center justify-between'>
        <div className='space-y-2'>
          <Skeleton className='w-24 h-6' />
          <Skeleton className='w-16 h-4' />
        </div>
        <Skeleton className='size-12' />
      </CardHeader>
      <CardContent>
        <Skeleton className='shrink-0 w-24 h-10 mb-2' />
        <Skeleton className='shrink-0 w-40 h-4' />
      </CardContent>
    </Card>
  );
};
